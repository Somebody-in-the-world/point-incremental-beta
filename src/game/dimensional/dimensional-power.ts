...
export const DimensionalPower = new (class extends Currency {
    ...
    private get rawEffect() {
        ...
        if (SpacetimeChallenges.dimPowExp.running) return effect;
        if (TearSpacetimeUpgrades.dimPowerFormula.bought) {
            effect = Decimal.max(this.add(1).log10().pow(2).div(6000), effect);
        }
    ...
    get effect() {
        if (SpacetimeChallenges.dimPowMult.running) return new Decimal(0);
